...
t1 = time.time()
print('mnist dataset loaded in: %.2f' % (t1-t0))

print('Loading model')
t0 = time.time()
autoencoder = load_model('autoencoder_digits.h5')
...
def retrieve_closest_images(test_element, test_label, n_samples=10):
    ...
    print("Average ranking Precision score for tested element is {}".format(score))

    original_image = x_test[25]
...
def test_model(n_test_samples, n_train_samples):
    ...
    print('Starting Computing Score....')
    t1 = time.time()
    score = compute_average_precision_score(test_codes[indexes], y_test[indexes], learned_codes, n_train_samples)
    t2 = time.time()
    print('Score computed in: %.2f seconds ' % (t2-t1))
    # print('Model percision score:', score)


def plot_denoised_images():
    denoised_images = autoencoder.predict(x_test_noisy.reshape(x_test_noisy.shape[0], x_test_noisy.shape[1], x_test_noisy.shape[2], 1))
    test_img = x_test_noisy[25]
    resized_test_img = cv2.resize(test_img, (280, 280))
    cv2.imshow('input', resized_test_img)
    cv2.waitKey(0)
    output = denoised_images[25]
...
retrieve_closest_images(x_test[25], y_test[25])
